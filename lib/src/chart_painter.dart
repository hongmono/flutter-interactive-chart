...
class ChartPainter extends CustomPainter {
  ...
  void _drawTapHighlightAndOverlay(canvas, PainterParams params) {
    ...
    final vLine = params.getCandleIndexFromOffset(pos.dx);
    final candle = params.candles[vLine];
    canvas.save();
    canvas.translate(params.xShift, 0.0);

    // Draw highlight bar (selection box)
    canvas.drawLine(
        Offset(vLine * params.candleWidth, 0.0),
        Offset(vLine * params.candleWidth, params.chartHeight),
        Paint()
    ...
    final priceTp = TextPainter(
      text: TextSpan(
        text: getPriceLabel(((params.maxPrice - params.minPrice) * (1 - pos.dy / params.chartHeight)) + params.minPrice),
        style: params.style.priceLabelStyle,
      ),
    )
      ..textDirection = TextDirection.ltr
    ...
    priceTp.paint(
        ...
        Offset(
          params.chartWidth + 4,
          params.fitPrice(((params.maxPrice - params.minPrice) * (1 - pos.dy / params.chartHeight)) + params.minPrice) - priceTp.height / 2,
        ));
